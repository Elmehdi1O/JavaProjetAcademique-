...
public class ModeleVueDonneesProjets {
	...
	public boolean rechercheIdProjet() {
		...
		try {
			...
			System.err.println("id de projet doit etre un entier!!");
			return false;
		}
		...
		projet = tableProjet.searchById(id_projet);

		return true;
	}
	
	public Etudiant searchEtudiantByIdProjet(int id_projet) {
		ArrayList<Etudiant> listEtudiants = tableEtudiant.getAll();
		for(Etudiant e:listEtudiants) {
			if(e.getId_projet()==id_projet)
				return e;
		}
		return null;
	}
	...
	public void supprimerProjet() {
		...
		Etudiant etudiant = searchEtudiantByIdProjet(id);
		etudiant.setId_projet(null);
		etudiant.setId_professeur(null);
		tableEtudiant.update(etudiant.getId_etudiant(), etudiant);
