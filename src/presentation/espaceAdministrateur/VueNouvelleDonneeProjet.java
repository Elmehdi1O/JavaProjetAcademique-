...
public class VueNouvelleDonneeProjet extends JFrame{
	...
	public void init() {
		...
		Projet p = modele.getProjet();
		Etudiant e = modele.searchEtudiantByIdProjet(p.getId_projet());

		String cne = e.getCNE();
		
		theme_projetText = new JTextField(p.getTheme_projet());
		cneText = new JTextField(cne);
		id_professeurText = new JTextField(String.valueOf(p.getId_professeur()));  
		typelist = new JComboBox<String>(new String[] {"Fin d'année", "Fin d'étude","Doctorat"});
		typelist.setSelectedItem(p.getType());
		clotureText = new JTextField(p.getCloture());
		dureeText = new JTextField(p.getDuree());
